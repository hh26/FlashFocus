...
export default function AIGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      Each object must have exactly these keys: "question", "answer", and "tag". 
      The "tag" must be a single, highly relevant string (e.g. "biology", "history") representing the specific sub-topic.`;
      ...
      const formattedCards = generatedData.map((card: any) => ({
        question: card.question,
        answer: card.answer,
        tags: [card.tag.toLowerCase().replace(/[^a-z0-9-]/g, '')], // Ensure tag is clean and wrapped in array
      ...
      setTopic('');
      alert(`Successfully generated and saved ${formattedCards.length} cards using ${AGENTS[selectedAgent].name}!`);

    } catch (err: any) {
      setError(err.message || 'Failed to generate cards. The AI returned invalid formatting.');
    } finally {
